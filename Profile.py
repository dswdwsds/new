...
# ✅ تعديل: استخراج anime_id عن طريق الضغط على زر داخل صفحة الحلقة
def extract_anime_id_from_custom_url(custom_url):
    try:
        response = scraper.get(custom_url)
        if response.status_code != 200:
            print(f"❌ فشل تحميل صفحة الحلقة: {custom_url}")
            return None

        tree = html.fromstring(response.content)
        anime_page_link = tree.xpath("/html/body/div[4]/div[2]/div[2]/div/div[2]/div/a/@href")
        if not anime_page_link:
            print("❌ لم يتم العثور على رابط صفحة الأنمي في الزر.")
            return None

        anime_url = anime_page_link[0]
        print(f"🔗 تم استخراج رابط صفحة الأنمي: {anime_url}")
        ...
        if len(parts) >= 2 and parts[0] == "anime":
            ...
            print("❌ فشل استخراج anime_id من رابط الأنمي.")
            return None

    except Exception as e:
        print(f"❌ خطأ أثناء معالجة رابط الحلقة: {e}")
        return None
...
def fetch_anime_info(anime_id):
    ...
    if response.status_code != 200:
        print(f"❌ فشل تحميل صفحة الأنمي: {anime_url}")
        return None
