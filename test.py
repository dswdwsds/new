...
def update_new_json_list(anime_title, ep_data):
    new_json_url = f"https://abdo12249.github.io/1/test1/المشاهده.html?id={to_id_format(anime_title)}&episode={ep_data['number']}"
    api_url = f"https://api.github.com/repos/{repo_name}/contents/test1/new2.json"
    headers = {"Authorization": f"token {access_token}"}
    response = scraper.get(api_url, headers=headers)
    sha = None
    data = []
    ...
    if response.status_code == 200:
        ...
        print("⚠️ فشل في قراءة ملف الجديد.json")
        return

    # البحث عن الأنمي داخل الملف
    anime_found = False
    for anime in data:
        if anime.get("animeTitle") == anime_title:
            if not any(str(e["number"]) == str(ep_data["number"]) for e in anime["episodes"]):
                anime["episodes"].insert(0,{
    ...
    payload = {
        "message": f"تحديث ملف الجديد.json بإضافة {anime_title} الحلقة {ep_data['number']}",
        "content": encoded_content,
        "branch": "main"
    }
    if sha:
    ...
    if r.status_code in [200, 201]:
        print(f"✅ تم تحديث الجديد.json بـ {anime_title} الحلقة {ep_data['number']}")
    else:
        print(f"❌ فشل تحديث الجديد.json: {r.status_code} {r.text}")
    ...
    if anime_name and server_list:
        ...
        if status in ["new", "update"]:
            ...
            if status == "new":
                ...
                update_new_json_list(anime_name, ep_data)
    else:
